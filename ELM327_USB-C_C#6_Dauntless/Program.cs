        ...
        foreach (string port in ports)
        {
            using (SerialPort serialPort = new SerialPort(port))
            {
                ...
                try
                {
                    ...
                    if (File.Exists("ELM327_commands_Dauntless.txt"))
                    {
                        string[] atParancsok = File.ReadAllLines("ELM327_commands_Dauntless.txt");

                        using (StreamWriter sw = new StreamWriter(responseFile, false))
                        {
                            // Egy 'üres' karaktert küldünk (csak egy carriage return-t), hogy kiváltsuk az eszköz ön-riportját
                            Console.WriteLine("Küldés: üres parancs az eszköz inicializálásához...");
                            serialPort.Write("\r");

                            // Olvassuk és összegyűjtjük az eszköz összes ön-riport sorát, sőt [INIT]-tel megjelöljük
                            var initialDump = new List<string>();
                            DateTime start = DateTime.Now;
                            while ((DateTime.Now - start).TotalMilliseconds < serialPort.ReadTimeout)
                            {
                                try
                                {
                            ...
                            // Loggoljuk és elmentjük az ön-riportot
                            Console.WriteLine("<<< Eszköz önriport:");
                            foreach (var line in initialDump)
                            {
                                Console.WriteLine(line);
                                sw.WriteLine($"[INIT] {line}");
                            }
                            foreach (string parancs in atParancsok)
                            {
                                ...
                                try
                                {
                                    // Összegyűjti a sorokat, amíg egy végkarakter vagy timeout nem történik
                                    var responseLines = new List<string>();
                                    DateTime cmdStart = DateTime.Now;
                                    while ((DateTime.Now - cmdStart).TotalMilliseconds < serialPort.ReadTimeout)
                                    {
                                        string line = serialPort.ReadLine();
                                        if (string.IsNullOrWhiteSpace(line))
                                            break;
                                    ...
                                    string valasz = string.Join(" | ", responseLines);
                    ...
                    {
                        Console.WriteLine("Az 'ELM327_commands_Dauntless.txt' fájl nem található.");
                        Console.ReadKey();
                    }
